...
public class ImageResizer {
	 ...
	 */
	public static void resize(File origFile, int topsize, boolean centered, File destfile, String type)
			throws IOException {
	 ...
	 *             Signals that an I/O exception has occurred.
	 */
	public static boolean shrinkTo(File origFile, int thresholdWidth, int thresholdHeight, File destfile, String type)
			throws IOException {
		int newWidth = 0;
		int newHeight = 0;
		boolean ret = false;
	 ...
	 *             Signals that an I/O exception has occurred.
	 */
	public static void resize(File origFile, int maxWidth, int maxHeight, boolean centered, File destfile, String type)
			throws IOException {
	 ...
	 *             Signals that an I/O exception has occurred.
	 */
	public static void threshold(File origFile, int maxWidth, int maxHeight, boolean centered, File destfile,
			String type, Color color) throws IOException {
		BufferedImage bi = ImageIO.read(origFile);
		int calcWidth;
		int calcHeight;
		...
		if (width > maxWidth || height > maxHeight) {
			...
			ImageIO.write(createResizedCopy(bi, maxWidth, maxHeight, centered, calcWidth, calcHeight, color), type,
					destfile);
		} else {
	...
	private static BufferedImage createResizedCopy(Image originalImage, int scaledWidth, int scaledHeight,
			boolean centered, int topWidth, int topHeight) {
	 ...
	 */
	private static BufferedImage createResizedCopy(Image originalImage, int scaledWidth, int scaledHeight,
			boolean centered, int topWidth, int topHeight, Color color) {
	 ...
	 *             Signals that an I/O exception has occurred.
	 */
	public static void resizeCrop(File origFile, int maxWidth, int maxHeight, File destfile, String type)
			throws IOException {
		BufferedImage bi = ImageIO.read(origFile);
	 ...
	 * @return the buffered image
	 */
	private static BufferedImage createResizedCropCopy(Image originalImage, int scaledWidth, int scaledHeight,
			boolean centered, int topsize) {
		...
		g.setColor(Color.WHITE);
		g.fillRect(0, 0, topsize, topsize);
		g.drawImage(originalImage, 0, 0, topsize, topsize, scaledWidth, scaledHeight,
				originalImage.getWidth(null) - scaledWidth, originalImage.getHeight(null) - scaledHeight, Color.WHITE,
				null);
		g.dispose();
		return scaledBI;
	}
	 ...
	 */
	private static BufferedImage createResizedCropCopy(Image originalImage, int scaledWidth, int scaledHeight,
			int topWidth, int topHeight, float factor) {
		...
		g.drawImage(originalImage, 0, 0, topWidth, topHeight, x1, y1, x2, y2, Color.WHITE, null);
		g.dispose();
		return scaledBI;
	}
}
