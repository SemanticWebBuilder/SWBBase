...
public class AutoStatement implements java.sql.Statement {
	...
	/** The log. */
	private static final Logger LOG = SWBUtils.getLogger(AutoStatement.class);
	...
	private void checkStatement() throws SQLException {
		if (debug) {
			LOG.debug("s.checkStatement");
		}
		if (!closed && aconn.getNativeConnection() != conn) {
			if (conn != null) {
				LOG.info("Recreating Statement...");
			}
	...
	public java.sql.ResultSet executeQuery(String str) throws java.sql.SQLException {
		if (debug) {
			LOG.debug("s.executeQuery:" + str);
		}
		checkStatement();
		try {
			return st.executeQuery(str);
		} catch (SQLException se) {
			LOG.error("Error conexion dañada..., " + se);
			if (se.getMessage().toLowerCase().contains("socket")) {
				aconn.changeConnection();
				checkStatement();
				return st.executeQuery(str);
	...
	public int getResultSetConcurrency() throws java.sql.SQLException {
		if (debug) {
			LOG.debug("s.getResultSetConcurrency");
		}
		checkStatement();
	...
	public void setEscapeProcessing(boolean param) throws java.sql.SQLException {
		if (debug) {
			LOG.debug("s.setEscapeProcessing");
		}
		checkStatement();
	...
	public int getResultSetHoldability() throws java.sql.SQLException {
		if (debug) {
			LOG.debug("s.getResultSetHoldability");
		}
		checkStatement();
